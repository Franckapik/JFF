 ...
 *   node scripts/test-fsm-autonomous.js
 *   node scripts/test-fsm-autonomous.js --verbose
...
import { makeInitialContext } from './fsm-mock-data.js';
...
async function runAutonomousTest() {
  try {
    ...
    // Démarrer le tracker simulé
    const tracker = new SimulatedTracker(actor, { verbose });
    const unsubscribeTracker = tracker.start();

    // Démarrer la machine
    console.log('🚀 Starting autonomous FSM with simulated tracker...\n');
    actor.start();
