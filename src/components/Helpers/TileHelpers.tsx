...
const TileHelpers: React.FC<TileHelpersProps & { coord?: string }> = ({
  position,
  tileType,
  ...
  // Affichage temporaire des coordonnées grid au début du jeu
  const [showGridCoord, setShowGridCoord] = React.useState(true);
  React.useEffect(() => {
    const timer = setTimeout(() => setShowGridCoord(false), 20000);
    return () => clearTimeout(timer);
  }, []);
  return (
    <>
      {/* Overlay Html pour afficher les coordonnées grid pendant 10s */}
      {showGridCoord && coord && (
        <Html position={[position[0], 0.1, position[2]]} center distanceFactor={20} zIndexRange={[100, 0]}>
          <div style={{
            color: '#fff',
            padding: '2px 8px',
            borderRadius: '4px',
